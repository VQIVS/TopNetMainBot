...
def support(message):
    user_id = message.from_user.id
    support_message = """🟦لطفا هرروز اشتراک خود را مطابق این پست آپدیت کنید: https://t.me/top_netvpn/68
📞برای رفع مشکل یا صحبت با پشتیبانی به آیدی مقابل پیام دهید: @TopNetSupport
...
def how_to_connect(message):
    user_id = message.from_user.id
    text = """🟦لطفا هرروز اشتراک خود را مطابق این پست آپدیت کنید: https://t.me/top_netvpn/68
✅برای دانلود نرم افزار، راهنمای وارد کردن لینک و اتصال، مطالب کانال زیر را مشاهده کنید: @TopNetHelper
❎لطفا دقت کنید اسم کانفیگ اول اشتراکتون بعد از هر بار آپدیت اشتراکتون، حجم و زمان باقیماندتون رو نمایش میدهد.
"""
    bot.send_message(user_id, text)
...
در غیر این صورت روی گزینه پرداخت کلیک کنید.✅در صورتی که خودتان درگاه ارز دیجیتال یا کیف پول میشناسید و استفاده میکنید، میتوانید با استفاده از آن کیف پول واریز نمایید.
